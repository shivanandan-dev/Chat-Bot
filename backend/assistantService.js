...
app.post('/v1/create-conversation', (req, res) => {
    ...
    const newConversation = { id: uuidv4(), title: title ?? "New chat", createdTime: new Date().toISOString(), updatedTime: new Date().toISOString() };
...
app.post('/v1/update-conversation', (req, res) => {
    ...
    const conversationIndex = dummyChatListData.conversations.findIndex((conversation) => conversation.id === id);
...
app.delete('/v1/conversations/:id', (req, res) => {
    const { id } = req.params;
    dummyChatListData.conversations = dummyChatListData.conversations.filter((conversation) => conversation.id !== id);
    res.json({ success: true });
});
...
app.post('/v1/create-message', (req, res) => {
    ...
    const userMessage = { id: uuidv4(), type: "user", message: userQuery, createdTime: new Date().toISOString() };
        ...
        assistantMessages.forEach((msg, index) => {
            setTimeout(() => {
                ...
                if (index === assistantMessages.length - 1) {                    
                    ...
                    const assistantMessage = { id: uuidv4(), type: "assistant", message: assistantStreamBuffer, createdTime: new Date().toISOString() };
